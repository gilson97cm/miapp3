...
public class CompassView extends View {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      //  mCanvas.drawColor(Color.BLACK);
        ...
        //Dibuja los dos arcos exteriores
        drawOutSideArc();

        // Dibujar un triangulo gris
        drawGrayTriangle();

        // Dibuja el valor del ángulo medio
        drawMidAngle();
        ...
        // Dibuje la línea de escala, el valor de escala y la dirección en el dial
        drawOthers();
    }
    ...
    private void drawOutSideArc() {
        ...
        outsideArcPaint.setColor(Color.argb(255, 0, 0, 0));
    ...
    private void drawMidAngle() {
        ...
        Typeface typeface = Typeface.createFromAsset(getContext().getAssets(), "fonts/fontnumber_light.ttf"); //hay que agregar las fuentes
        //en project->main-> asstets->fonts
        midAnglePaint.setTypeface(typeface);
        midAnglePaint.setAntiAlias(true);
        midAnglePaint.setColor(Color.argb(255, 0, 0, 0));
    ...
    private void drawInSideArc() {
        ...
        // El ángulo recogido por el sensor de dirección del teléfono móvil es en sentido antihorario y girado en sentido horario para facilitar el dibujo.
        float angle = 360 - directionAngle;
        // El tamaño del espacio superior en el medio del anillo.
        float space = 1;
    ...
    private void drawRedTriangle() {
        // Dibuja un triángulo negro ligeramente más grande detrás del triángulo rojo para lograr el efecto de relleno.
    ...
    private void drawOthers() {
        ...
        // Este-oeste, sur-norte en círculo circular
        Paint dirTextPaint = new Paint();
        dirTextPaint.setAntiAlias(true);
        dirTextPaint.setTextSize(textDirSize);
        // 30 valores enteros de escala múltiple en el círculo de marcación
        ...
        for (int i = 0; i < 360; i++) {
            if (i % 90 == 0) {
                ...
                dirTextPaint.setColor(Color.argb(255, 0, 0, 0));
                String text = "Norte";
                ...
                if (i == 0) {
                    ...
                    dirTextPaint.setColor(Color.BLACK);
                    if ((i == 90)) {
                        String direction = "Este";
                        mCanvas.drawText(direction, -textWidth / 2, textHeight - inOvalSize + scaleLength + (int) (spaceSize * 1.6 + 0.5), dirTextPaint);
                    } else if (i == 180) {
                        String direction = "Sur";
                        mCanvas.drawText(direction, -textWidth / 2, textHeight - inOvalSize + scaleLength + (int) (spaceSize * 1.6 + 0.5), dirTextPaint);
                    } else if (i == 270) {
                        String direction = "Oeste";
                        mCanvas.drawText(direction, -textWidth / 2, textHeight - inOvalSize + scaleLength + (int) (spaceSize * 1.6 + 0.5), dirTextPaint);
                    }
                }
            } else if (i % 30 == 0) {
